        ...
        messages = [
            {
                "role": "system",
                "content": f"""
                       你现在是一位小说文案师，你需要学习并精炼以上内容，以写小说故事的形式，写一份字数1100字左右的视频故事，请严格执行一下要求：
                        1、视频故事的开头要有爆点，有吸引力；
                        2、视频故事的结尾内容不变；
                        3、视频故事中不要有任何对话；
                        4、视频故事的不需要分段，以连续的故事内容展示；
                        5、视频故事的字数要在1100个字左右；
                        6、禁止出现性暗示词汇和语句；【禁止带明显倾向或暗示的以及情色类的句子】
                        7、禁止出现人体实验，替换成科学实验。
                        8、形容词删除掉
                        9禁止出现任何暧昧场景，性暗示，色情这些场景
                        10、罪犯可以用嫌疑人替代或者其他词，通缉犯可以TJ字母替代，监狱改用牢狱
                        11、禁止出现血腥暴力暴露的文字 不要出现对话、拥抱、牵手、亲吻、与军队有关的情节 不要出现裸露身体的场景 都穿长衣长裤 故事开头一段以风景为主
                        12、结尾留悬念。重点
                        
                        **输出格式**：
                        请返回一个JSON对象，将1100字左右的视频故事分段，为每段内容生成包含一个名为"plot_titles"的数组，数组中包含多个对象，每个对象都要包含以下字段：
                       {{
                           "plot_titles": [
                               {{
                                   "background": "本段视频故事的场景描写，并加入提示词强调图片风格为{video_style}",
